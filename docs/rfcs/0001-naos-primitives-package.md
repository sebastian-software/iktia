# RFC 0001: Naos Primitives Package
...
Add a separate `@naos-ui/primitives` package for core UI primitives authored in
Naos and shipped as native Web Components.
...
expectations, but Naos primitives must use Naos's own authoring model,
...
* Ship a real public package named `@naos-ui/primitives`.
* Provide a small but useful baseline of accessible Web Component primitives.
* Exercise Naos's compiler with production-shaped components before a larger
...
* Do not add a virtual DOM, reconciler, hook runtime, or framework
  compatibility layer.
* Do not move primitive-specific behavior into `@naos-ui/runtime`.
* Do not make advanced overlay positioning a prerequisite for the first package
  milestone.
* Do not hide native platform semantics behind Naos-specific abstractions when
  slots, attributes, properties, forms, ARIA, or browser APIs are enough.

## Existing Constraints

This package must follow the accepted Naos architecture:
...
The new public package is `@naos-ui/primitives`.

Component exports use branded PascalCase names so the generated Custom Element
tags are stable and clearly owned by Naos:

| Export | Custom Element | Stability At Introduction |
| --- | --- | --- |
| `NaosButton` | `<naos-button>` | Experimental |
| `NaosButtonGroup` | `<naos-button-group>` | Experimental |
| `NaosField` | `<naos-field>` | Experimental |
| `NaosInput` | `<naos-input>` | Experimental |
| `NaosTextarea` | `<naos-textarea>` | Experimental |
| `NaosCheckbox` | `<naos-checkbox>` | Experimental |
| `NaosToggle` | `<naos-toggle>` | Experimental |
| `NaosRadioGroup` | `<naos-radio-group>` | Experimental |
| `NaosRadio` | `<naos-radio>` | Experimental |
| `NaosTabs` | `<naos-tabs>` | Experimental |
| `NaosTabList` | `<naos-tab-list>` | Experimental |
| `NaosTab` | `<naos-tab>` | Experimental |
| `NaosTabPanel` | `<naos-tab-panel>` | Experimental |
| `NaosDisclosure` | `<naos-disclosure>` | Experimental |
| `NaosAccordion` | `<naos-accordion>` | Experimental |
| `NaosDropdown` | `<naos-dropdown>` | Experimental |
| `NaosMenu` | `<naos-menu>` | Experimental |
| `NaosMenuItem` | `<naos-menu-item>` | Experimental |
| `NaosPopover` | `<naos-popover>` | Later candidate |
| `NaosTooltip` | `<naos-tooltip>` | Later candidate |
| `NaosDialog` | `<naos-dialog>` | Later candidate |
...
import { NaosButton } from "@naos-ui/primitives"
```
...
`@naos-ui/primitives/internal/behavior/*` during the experimental phase. Public
...
Naos primitives should follow the Remix v3-inspired "use the platform" rule:
...
semantics instead of replacing them with synthetic abstractions. Naos-specific
...
Events use an Naos-prefixed convention:

| Event | Purpose |
| --- | --- |
| `naos-change` | A component value or checked/pressed/selected state changed. |
| `naos-open-change` | An open/closed state changed. |
| `naos-select` | A menu or list item was selected. |
| `naos-input` | Text input changed while editing. |
| `naos-invalid` | Validation state changed or validation failed. |
...
interaction logic separate from visual structure. Naos should not copy
Downshift's React hook or getter-prop API, because Naos components compile to
native Custom Elements instead of rendering through React.

The Naos translation is a behavior kernel:
...
Behavior kernels belong in `@naos-ui/primitives`, not `@naos-ui/runtime`.
`@naos-ui/runtime` remains limited to generated-code platform helpers. Tabs,
...
Naos should avoid inventing complex behavior where mature prior art exists.
...
* **Adapt**: wrap a framework-agnostic package behind Naos's behavior-kernel
  interface.
...
* **Reference only**: use the source for taxonomy, edge cases, docs, or tests,
  but write Naos behavior independently.
...
* **Runtime-boundary fit**: whether reuse would keep primitive semantics inside
  `@naos-ui/primitives` and avoid growing `@naos-ui/runtime`.
...
| API lock-in risk | Would leak upstream API | Can be hidden with effort | Naturally hidden behind Naos kernels |
...
| [Downshift](https://www.downshift-js.com/) | Select and combobox interaction model, getter-prop ergonomics, ARIA behavior, controlled/uncontrolled state ideas. | Reference or clean-room port for select/combobox behavior. | React hook/getter-prop API does not map directly to Naos components. |
...
Preliminary decisions for `@naos-ui/primitives`:

* **Direct dependency candidate**: `@floating-ui/dom` or `@floating-ui/core` for
  positioning only, starting in M5. Keep menu/listbox/dialog state in Naos
  behavior kernels.
...
* **Clean-room/reference candidates**: Downshift and React Aria for select and
  ...
  collections, and component tree assumptions do not fit Naos directly.
* **Reference-only candidates**: Radix UI and Ariakit for taxonomy, focus
  ...
  is too far from Naos's Custom Element output for direct reuse.
...
and tabs gaps with private Naos behavior kernels.

Reasons:

* The current missing behavior is small enough to cover with deterministic
  transition functions and browser tests.
* Zag machines would still require an Naos-specific Custom Element and Shadow
...
* the machine API leaks through public `@naos-ui/primitives` exports;
...
* A Custom Element adapter can translate Zag events such as `ARROW_NEXT`,
  `ARROW_PREV`, `HOME`, `END`, `TAB_FOCUS`, and `TAB_CLICK` into Naos state
  transitions, but that adapter becomes real primitive infrastructure rather
  than a tiny drop-in dependency.
* The current tabs behavior remains smaller as an Naos-owned kernel. Zag should
...
do not wire the current `<naos-tabs>` implementation to Zag yet. Re-evaluate
...
* **Button, field, input, textarea**: use native controls and Naos styling
...
* **Checkbox and toggle**: start native/internal. Compare Web Awesome/Shoelace
  ...
  cases. A small Naos toggle kernel is acceptable if Form-Associated Custom
  Elements require it.
...
* **Disclosure/accordion**: start with native `<details>` as the baseline, then
  ...
  Naos-owned kernel unless native behavior satisfies state reflection and
  styling needs.
...
* Prefer direct dependency only when the imported module is framework-agnostic,
  small, license-compatible, tree-shakable, and does not violate Naos's runtime
  boundary.
* Prefer adaptation when the source is architecturally strong but its public API
  should be hidden behind Naos's behavior-kernel interface.
...
* Do not expose third-party API shapes directly from `@naos-ui/primitives` unless
...
`NaosButton` wraps button semantics with slot-based content, disabled state,
...
`NaosButtonGroup` groups action controls and exposes orientation state. It does
not manage selection unless paired with a future segmented control primitive.

### Forms

`NaosField` composes label, hint, error, and control slots. It owns layout and
description wiring but should not obscure the underlying input semantics.

`NaosInput` and `NaosTextarea` provide styled native text-entry controls with
value, disabled, invalid, required, placeholder, and name behavior.

`NaosCheckbox` exposes checked, unchecked, and indeterminate states. It must
support keyboard toggling, label composition, disabled state, and form behavior.

`NaosToggle` covers switch/pressed controls. It should expose `on` and `off`
states with `aria-pressed` or `role="switch"` depending on the final semantic
choice made during implementation.

`NaosRadioGroup` and `NaosRadio` provide single selection, roving keyboard
navigation, disabled items, orientation, and form behavior.

### Navigation And Disclosure

`NaosTabs`, `NaosTabList`, `NaosTab`, and `NaosTabPanel` provide selected
tab state, panel association, orientation, keyboard navigation, and meaningful
static HTML before upgrade.

`NaosDisclosure` provides a single trigger and panel with open/closed state.

`NaosAccordion` composes multiple disclosure items and defines whether single
or multiple panels may be open.

### Menus And Overlays

`NaosDropdown`, `NaosMenu`, and `NaosMenuItem` are the first overlay-like
...
`NaosPopover`, `NaosTooltip`, and `NaosDialog` are later candidates. Prefer
...
* experimental form primitives until Naos has enough support to guarantee
...
The MVP is implemented for `<naos-checkbox>` and `<naos-toggle>` with `name`,
...
* Add `@naos-ui/primitives` to the workspace package set and release config.
...
* Implement `NaosButton`.
* Implement `NaosButtonGroup`.
* Implement `NaosField`.
* Implement `NaosInput`.
* Implement `NaosTextarea`.
* Implement `NaosCheckbox`.
* Implement `NaosToggle`.
...
* Decide the form participation model for `NaosInput`, `NaosTextarea`,
  `NaosCheckbox`, `NaosRadioGroup`, and future select-like primitives.
...
* Implement `NaosTabs`, `NaosTabList`, `NaosTab`, and `NaosTabPanel`.
* Implement `NaosDisclosure`.
* Implement `NaosAccordion`.
* Implement `NaosRadioGroup` and `NaosRadio` if not completed in M3.
...
* Implement `NaosDropdown`, `NaosMenu`, and `NaosMenuItem`.
...
* Add focus return, Escape, outside click, item selection, and keyboard
  navigation coverage.
* Keep `NaosPopover`, `NaosTooltip`, and `NaosDialog` as later candidates
...
custom properties. Internal generated `data-naos-*` hydration markers remain
...
`@naos-ui/primitives`; `@naos-ui/runtime` remains reserved for generated-code
platform helpers.
...
`@naos-ui/primitives` focused on behavior, contracts, and minimal defaults.
